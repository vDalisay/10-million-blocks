...
# Make the pickup treatment unmistakably different from world blocks: half-opacity holographic blocks
# with stronger scanline/subpixel modulation. This remains one cached material per block type.
path = "src/Collection/ResourceCollectionField.cs"
replace_once(path, 'uniform float opacity = 0.80;', 'uniform float opacity = 0.50;')
replace_once(path, 'uniform float crt_strength = 0.42;', 'uniform float crt_strength = 0.72;')
replace_once(path, 'float scan = mod(floor(FRAGCOORD.y), 2.0) < 1.0 ? 0.62 : 1.0;', 'float scan = mod(floor(FRAGCOORD.y), 2.0) < 1.0 ? 0.42 : 1.0;')
replace_once(
    path,
...
replace_once(path, 'float flicker = 0.985 + 0.015 * sin(TIME * 18.0 + FRAGCOORD.y * 0.13);', 'float flicker = 0.965 + 0.035 * sin(TIME * 18.0 + FRAGCOORD.y * 0.13);')
replace_once(path, 'EMISSION = ALBEDO * (0.035 * crt_strength);', 'EMISSION = ALBEDO * (0.060 * crt_strength);')
replace_once(path, 'material.SetShaderParameter("opacity", 0.80f);', 'material.SetShaderParameter("opacity", 0.50f);')
replace_once(path, 'material.SetShaderParameter("crt_strength", 0.42f);', 'material.SetShaderParameter("crt_strength", 0.72f);')


# Phase Q: polish the ordinary and Steam-demo completion state without changing navigation semantics.
world_complete = r'''using System;
using Godot;
...
'''
(ROOT / "src/UI/WorldCompleteView.cs").write_text(world_complete, encoding="utf-8")

# Keep the status document truthful about the non-local Phase Q work that has now moved forward.
status_path = ROOT / "docs/IMPLEMENTATION_STATUS.md"
status = status_path.read_text(encoding="utf-8")
anchor = "- distinct `STEAM DEMO COMPLETE` flow after the 50³ finale."
replacement = (
    "- distinct `STEAM DEMO COMPLETE` flow after the 50³ finale, now with a dedicated dark-space finale panel, "
    "full 1³→50³ route marker, source breakdown and restrained entrance motion."
)
if anchor not in status:
    raise RuntimeError("implementation status completion anchor not found")
status_path.write_text(status.replace(anchor, replacement, 1), encoding="utf-8")

print("Applied pickup CRT/opacity tuning and Phase Q finale polish.")
